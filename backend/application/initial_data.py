...
with app.app_context():
    ...
    if not datastore.find_user(email = "customer@email.com"):
        datastore.create_user(name = "Customer" , email = "customer@email.com", password = hash_password("customer123") ,roles=["customer"])
    
    if not datastore.find_user(email = "professional@email.com"):
        datastore.create_user(name = "Professional" , email = "professional@email.com", password = hash_password("professional123") ,roles=["professional"])
